...
class Keyboard:
    ...
    def key_down(self, key):
        inputs = Inputs()
        inputs.ki = KeyboardInput(0, SCAN_CODES[key.upper()], 0x0008, 0, ctypes.pointer(ctypes.c_ulong(0)))
        input = Input(ctypes.c_ulong(1), inputs)
        self.api.SendInput(1, ctypes.pointer(input), ctypes.sizeof(input))

    def key_up(self, key):
        inputs = Inputs()
        inputs.ki = KeyboardInput(0, SCAN_CODES[key.upper()], 0x0008 | 0x0002, 0, ctypes.pointer(ctypes.c_ulong(0)))
        input = Input(ctypes.c_ulong(1), inputs)
        self.api.SendInput(1, ctypes.pointer(input), ctypes.sizeof(input))